...
var moveConfirm = function(url, id, authLevel){
	...
	if(url!=''){
		document.location.href="/KpcPersonalEvaluation"+url+"?employeId="+id+"&yyyyMM="+yyyyMM+"&authLevel="+authLevel;
	}
}
...
var checkLimit = function(attr){
	var totalLimit = 10;
	var id = attr.id.split("_")[0];
	var idLength = $('[id^="'+id+'"]').length;
	var total = 0;
...
var saveProjectData =  function(){
	...
	$.ajax({
		url : "/KpcPersonalEvaluation/saveProjectScore",
		type : "post",
		dataType : "text",
		data : {
			seq : seq,
...
var SavePerformData = function(authLevel){
	...
	if(valueComments=="" && parseInt(month.substr(4))%3==1){
...
var openDissentPop = function(id, month){
	window.open('/KpcPersonalEvaluation/popProjectDissent?yyyyMM='+month+'&employeId='+id,'dissentPop','width=750, height=400, scrollbars=no, resizable=no, status=no');
}
