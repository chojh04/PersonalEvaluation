	...
	<thead>						
		...
		<% for(int i=0; i<projectScoreList.size(); i++){
				...
				useAuth = useCheckBox(sessionEmployeId,
									  useAuthLevel,
									  sessionTeamCode,
									  projectScoreList.get(i).getTeamCode(),
		...
		<tr <%= ((projectScoreList.get(i).getStatus()==4 || projectScoreList.get(i).getStatus()==5) && useDissent)? "style=\"background-color:#FF9090\"" : "" %>>
			...
			<td>
				...
				%>				
				<% if( useDissent && todaydd <= limitDay ){ %>
			...
			%>	
				<td width="7%">
					<%if(useAuth && status<3 && todaydd <= limitDay){%> 
						<%if(k<8){%>
					...
					<% }else{
						...
						if(k<8 && projectScoreList.get(i).getEmployeId().equals(sessionEmployeId)){
							out.print("<font style='font-size:12px;'>("+myScore+")</font>");
							if(score > myScore){
...
<%!
	public boolean useCheckBox(String sessionId, int authLevel, int sessionTeamCode, int teamCode, String emplyeId, int status){
		boolean useAuth = false;		
		if(sessionId.equals(emplyeId) && status==0){
			useAuth = true;
		}else if(authLevel==1 && status==1 && (sessionTeamCode==0 || sessionTeamCode==teamCode)){
			useAuth = true;
		}else if(authLevel==2 && status==2 && (sessionTeamCode==0 || sessionTeamCode==teamCode)){
			useAuth = true;
		}
		return useAuth;
	}
	
	public boolean useDissent(String sessionId, int authLevel, int sessionTeamCode, int teamCode, String emplyeId, int status){
		boolean useDissent = false;		
		if(sessionId.equals(emplyeId) && status>=3){
			useDissent = true;
		}else if(authLevel==1 && status==4 && (sessionTeamCode==0 || sessionTeamCode==teamCode)){
			useDissent = true;
		}else if(authLevel==2 && status==5 && (sessionTeamCode==0 || sessionTeamCode==teamCode)){
			useDissent = true;
		}else if(authLevel>=1 && status==6){
			useDissent = true;
		}
		return useDissent;
	}
